...
| **AI 解析** | 智谱 AI（ZAI SDK）+ LangChain/LangGraph 文档入库智能体 |
| **AI 审核** | LangChain/LangGraph 多智能体审计 + 规则引擎兜底 |
...
│   │   ├── agents/               # LangChain/LangGraph PDF、审计、财务智能体
│   │   └── routes/
...
- 上传 PDF 文件（最大 25 MB）
- SHA-256 哈希去重
- LangChain/LangGraph 文档入库智能体编排 PDF 解析、字段校验、业务入库和自动审计
- `ParserStrategyAgent` 根据文件与解析质量选择解析工具，默认使用 `lite`，质量不足时自动升级到 `prime`
- `ExtractionQualityAgent` 根据文本长度、核心字段完整性决定继续、重试或标记为人工复核
- 智谱 AI 作为 PDF 解析工具，规则解析作为结构化抽取与兜底能力
- 异步任务轮询解析状态
...
- **业务审计**（`POST /api/ai-review/business/:id/run`）：
  - LangChain/LangGraph 审计主管智能体协调规则审计智能体与风险推理智能体
  - `AuditPlannerAgent` 根据字段缺失、费用明细、借贷差异和成本记录动态决定是否调用大模型风险推理
  - 借贷余额一致性
  - 费用明细汇总一致性
  - 核心字段完整性（发票号、DI 编号）
  - 退款金额异常检测
- **财务复核**（`POST /api/ai-review/cost-record/:id/review`）：
  - 财务复核智能体合并确定性规则校验与大模型风险解释
  - `FinancePlannerAgent` 根据数量、汇率、退款比例和明细完整性动态选择规则复核或 LLM 辅助分析
  - 总数量有效性
  - 汇率合理性（0 < 汇率 ≤ 20）
  - 退款与海关费比例
  - 单件成本 > 0
