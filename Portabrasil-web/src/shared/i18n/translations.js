export const TRANSLATIONS = {
  zh: {
    ...
    recent_uploads: '最近上传文件', verified: '已验证', ai_processing: 'AI 识别中...', parse_failed: '解析失败',
    upload_audit_done: (source, risk) => `解析完成，智能体审计：${source || '-'} / ${risk || '-'}`,
    ...
    advance_step: '推进下一步', rollback_step: '回退一步', process_control_hint: '流程控制会按顺序推进，回退时撤销最后一个已完成步骤。',
    process_no_permission: '当前角色无权操作该步骤',
    next_step_label: (name) => `下一步：${name || '-'}`,
    ...
    cost_business_link_title: '业务成本闭环', cost_select_business_title: '选择业务后核算成本', cost_select_business_desc: '系统会读取业务主表和费用明细，自动预填成本输入',
    cost_business_select_label: '关联业务记录', cost_business_select_placeholder: '选择 S/Ref / 客户 / 业务记录',
    cost_save_record: '保存成本记录', cost_save_success: '成本记录已保存', cost_record_note_auto: '由业务费用明细自动预填后保存', cost_record_note_manual: '手动成本记录',
    cost_records_title: '成本记录', cost_record_no: '记录编号', cost_no_records: '暂无成本记录',
    recent_records_title: '近期清关记录', details: '详情', afrmm: 'AFRMM 及其他',
    report_detail_title: '报表详情', report_progress: '流程进度',
    report_completed_steps: (done, total) => `已完成 ${done}/${total} 步`,
    report_cargo_summary: '货物摘要', report_arrival_date: '到港日期', report_destination: '目的地', report_weight: '毛重',
    report_fee_summary: '费用汇总', report_total_debit: '总借方', report_total_credit: '总贷方', report_balance: '余额',
    report_business_info: '业务资料', report_invoice_no: '发票号', report_customer: '客户', report_mawb_mbl: 'MAWB/MBL', report_hawb_hbl: 'HAWB/HBL', report_cif: 'CIF/BRL', report_data_status: '数据状态',
    report_process_steps: '清关步骤', report_fee_items: '费用明细', report_fee_date: '费用日期', report_fee_code: '费用代码', report_fee_name: '费用名称', report_credit: '贷方', report_debit: '借方', report_no_fee_items: '暂无费用明细',
    admin_title: '用户管理', admin_desc: '管理系统账号、角色和状态',
    admin_create: '新建账号', admin_edit: '编辑', admin_disable: '禁用', admin_enable: '启用',
    admin_reset_pwd: '重置密码', admin_reset_confirm: '确认重置', admin_reset_target: (username) => `是否重置‘${username}’该账号的密码`,
    admin_reset_self_blocked: '不能重置当前登录账号的密码',
    ...
    admin_enable_success: '账号已启用', admin_disable_success: '账号已禁用', admin_reset_success: '密码已重置为 123456',
    admin_manage_admin_blocked: '公司高管不能管理管理员账号',
    profile_change_password: '修改登录密码',
    profile_old_password: '原密码', profile_new_password: '新密码', profile_confirm_password: '确认新密码',
    profile_password_required: '请填写原密码和新密码', profile_password_logout_hint: '密码修改成功后需要重新登录',
    fetching_in: '获取中',
  },
  en: {
    ...
    recent_uploads: 'Recently Uploaded Files', verified: 'Verified', ai_processing: 'AI Processing...', parse_failed: 'Parse Failed',
    upload_audit_done: (source, risk) => `Parsed and audited by agent: ${source || '-'} / ${risk || '-'}`,
    ...
    advance_step: 'Advance Step', rollback_step: 'Rollback Step', process_control_hint: 'Process control advances in order and rolls back the last completed step.',
    process_no_permission: 'Current role cannot operate this step',
    next_step_label: (name) => `Next: ${name || '-'}`,
    ...
    cost_business_link_title: 'Business Cost Link', cost_select_business_title: 'Select a business to calculate cost', cost_select_business_desc: 'The system reads business data and fee items to prefill the cost input',
    cost_business_select_label: 'Linked Business', cost_business_select_placeholder: 'Select S/Ref / customer / business',
    cost_save_record: 'Save Cost Record', cost_save_success: 'Cost record saved', cost_record_note_auto: 'Saved after auto-prefill from business fee items', cost_record_note_manual: 'Manual cost record',
    cost_records_title: 'Cost Records', cost_record_no: 'Record No.', cost_no_records: 'No cost records yet',
    recent_records_title: 'Recent Clearance Records', details: 'Details', afrmm: 'AFRMM & Others',
    report_detail_title: 'Report Detail', report_progress: 'Process Progress',
    report_completed_steps: (done, total) => `${done}/${total} steps completed`,
    report_cargo_summary: 'Cargo Summary', report_arrival_date: 'Arrival Date', report_destination: 'Destination', report_weight: 'Gross Weight',
    report_fee_summary: 'Fee Summary', report_total_debit: 'Total Debit', report_total_credit: 'Total Credit', report_balance: 'Balance',
    report_business_info: 'Business Info', report_invoice_no: 'Invoice No.', report_customer: 'Customer', report_mawb_mbl: 'MAWB/MBL', report_hawb_hbl: 'HAWB/HBL', report_cif: 'CIF/BRL', report_data_status: 'Data Status',
    report_process_steps: 'Clearance Steps', report_fee_items: 'Fee Items', report_fee_date: 'Fee Date', report_fee_code: 'Fee Code', report_fee_name: 'Fee Name', report_credit: 'Credit', report_debit: 'Debit', report_no_fee_items: 'No fee items yet.',
    admin_title: 'User Management', admin_desc: 'Manage system users, roles and account status',
    admin_create: 'Create User', admin_edit: 'Edit', admin_disable: 'Disable', admin_enable: 'Enable',
    admin_reset_pwd: 'Reset Password', admin_reset_confirm: 'Confirm Reset', admin_reset_target: (username) => `Reset the password for '${username}'?`,
    admin_reset_self_blocked: 'You cannot reset your own password here',
    ...
    admin_enable_success: 'User enabled', admin_disable_success: 'User disabled', admin_reset_success: 'Password reset to 123456',
    admin_manage_admin_blocked: 'Company executives cannot manage administrator accounts',
    profile_change_password: 'Change Login Password',
    profile_old_password: 'Current password', profile_new_password: 'New password', profile_confirm_password: 'Confirm new password',
    profile_password_required: 'Please enter current and new password', profile_password_logout_hint: 'You will be signed out after changing your password',
  ...
  pt: {
    ...
    recent_uploads: 'Arquivos Recentes', verified: 'Verificado', ai_processing: 'Processando com IA...', parse_failed: 'Falha na análise',
    upload_audit_done: (source, risk) => `Analisado e auditado por agente: ${source || '-'} / ${risk || '-'}`,
    ...
    advance_step: 'Avançar Etapa', rollback_step: 'Voltar Etapa', process_control_hint: 'O controle avança em ordem e desfaz a última etapa concluída.',
    process_no_permission: 'A função atual não pode operar esta etapa',
    next_step_label: (name) => `Próxima: ${name || '-'}`,
    ...
    cost_business_link_title: 'Vínculo de Custo', cost_select_business_title: 'Selecione um processo para calcular custo', cost_select_business_desc: 'O sistema lê dados do processo e itens de taxa para preencher o custo',
    cost_business_select_label: 'Processo Vinculado', cost_business_select_placeholder: 'Selecionar S/Ref / cliente / processo',
    cost_save_record: 'Salvar Custo', cost_save_success: 'Registro de custo salvo', cost_record_note_auto: 'Salvo após preenchimento automático pelos itens de taxa', cost_record_note_manual: 'Registro manual de custo',
    cost_records_title: 'Registros de Custo', cost_record_no: 'No. Registro', cost_no_records: 'Nenhum registro de custo.',
    recent_records_title: 'Registros Recentes', details: 'Detalhes', afrmm: 'AFRMM e Outros',
    report_detail_title: 'Detalhe do Relatório', report_progress: 'Progresso',
    report_completed_steps: (done, total) => `${done}/${total} etapas concluidas`,
    report_cargo_summary: 'Resumo da Carga', report_arrival_date: 'Data de Chegada', report_destination: 'Destino', report_weight: 'Peso Bruto',
    report_fee_summary: 'Resumo de Taxas', report_total_debit: 'Total Débito', report_total_credit: 'Total Crédito', report_balance: 'Saldo',
    report_business_info: 'Dados do Processo', report_invoice_no: 'Fatura', report_customer: 'Cliente', report_mawb_mbl: 'MAWB/MBL', report_hawb_hbl: 'HAWB/HBL', report_cif: 'CIF/BRL', report_data_status: 'Status dos Dados',
    report_process_steps: 'Etapas Aduaneiras', report_fee_items: 'Itens de Taxa', report_fee_date: 'Data', report_fee_code: 'Código', report_fee_name: 'Nome da Taxa', report_credit: 'Crédito', report_debit: 'Débito', report_no_fee_items: 'Nenhum item de taxa.',
    admin_title: 'Gestão de Usuários', admin_desc: 'Gerencie usuários, papéis e status da conta',
    admin_create: 'Criar Usuário', admin_edit: 'Editar', admin_disable: 'Desativar', admin_enable: 'Ativar',
    admin_reset_pwd: 'Redefinir Senha', admin_reset_confirm: 'Confirmar', admin_reset_target: (username) => `Redefinir a senha da conta '${username}'?`,
    admin_reset_self_blocked: 'Você não pode redefinir sua própria senha aqui',
    ...
    admin_enable_success: 'Usuário ativado', admin_disable_success: 'Usuário desativado', admin_reset_success: 'Senha redefinida para 123456',
    admin_manage_admin_blocked: 'Executivos da empresa não podem gerenciar contas de administrador',
    profile_change_password: 'Alterar Senha de Login',
    profile_old_password: 'Senha atual', profile_new_password: 'Nova senha', profile_confirm_password: 'Confirmar nova senha',
    profile_password_required: 'Informe a senha atual e a nova senha', profile_password_logout_hint: 'Você sairá da sessão após alterar a senha',
